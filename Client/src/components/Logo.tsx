...
const Logo = ({ variant = 'horizontal' }: LogoProps) => {
     ...
     return (
          <div className={`flex ${isVertical ? 'flex-col' : 'items-center'} items-center gap-2.5`}>
               <span className={`${isVertical ? 'w-12 h-12' : 'w-10 h-10'} rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-violet-500/30 transition-all`}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={isVertical ? "size-10" : "size-8"}>
               ...
               <span className={`${isVertical ? 'text-xl' : 'text-lg'} font-bold text-gray-900 tracking-tight`}>
                    Split-Sync
               </span>
          </div>
     );
